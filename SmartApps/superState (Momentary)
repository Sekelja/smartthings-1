...
def cmdHandler(evt){
	...
	if (evt.value == "on" && isOverride){
        ...
        callingSwitch.overrideScene() //added by Giles
		//return
	} else
	if (evt.value == "on"){
  ...
  //  } else if (evt.value == "push"){
//		if (logInfo) log.info "cmdHandler- ${sceneID} on with push"
	//	stateMapsOK = setScene(sceneID,true,false)
		//return
	//}
...
def sceneSnap(sceneID,isNew) {
	def attributesToSave = ['hue', 'saturation', 'color', 'switch', 'level', 'whiteLevel', 'colorMode', 'colorTemperature']
